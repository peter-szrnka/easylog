...
class EnvironmentVariableTest {
    ...
    @ParameterizedTest
    @EnumSource(EnvironmentVariable.class)
    void testEnvironmentVariableValues(EnvironmentVariable variable) {
        ...
        switch (variable) {
            case SERVER_PORT -> {
                assertEquals("EASYLOG_SERVER_PORT", envName);
                assertEquals("8080", defaultValue);
            }
            case SSL_ENABLED -> {
                assertEquals("EASYLOG_SSL_ENABLED", envName);
                assertEquals("true", defaultValue);
            }
            case SERVICE_NAME -> {
                assertEquals("EASYLOG_SERVICE_NAME", envName);
                assertEquals("EasyLogService", defaultValue);
            }
            case SERVICE_TYPE -> {
                assertEquals("EASYLOG_SERVICE_TYPE", envName);
                assertEquals("easyLog", defaultValue);
            }
            case SERVER_DB_FILE -> {
                assertEquals("EASYLOG_SERVER_DB_FILE", envName);
                assertEquals("easylog.db", defaultValue);
            }
            case ENV -> {
                assertEquals("EASYLOG_ENV", envName);
                assertEquals("", defaultValue);
            }
            case SSL_KEYSTORE -> {
                assertEquals("EASYLOG_SSL_KEYSTORE", envName);
                assertNull(defaultValue);
            }
            case SSL_KEYSTORE_PASSWORD -> {
                assertEquals("EASYLOG_SSL_KEYSTORE_PASSWORD", envName);
                assertNull(defaultValue);
            }
        }
    }

    private String getFieldValue(EnvironmentVariable variable, String fieldName) {
        try {
            var field = EnvironmentVariable.class.getDeclaredField(fieldName);
